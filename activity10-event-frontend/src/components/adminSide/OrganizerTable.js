...
function OrganizerTable({ status = 'active', role = 'all', searchTerm = '', currentPage = 1, onTotalItemsChange = () => {} }) {
  ...
  const filteredOrganizers = useMemo(() => {
    return (allUsers || []).filter((organizer) => {
      const userRole = organizer.role?.toLowerCase();
      // Exclude admin users from User Management
      if (userRole === 'admin') return false;

      if (role !== 'all' && userRole !== role.toLowerCase()) return false;
  ...
  React.useEffect(() => {
    onTotalItemsChange(filteredOrganizers.length);
  }, [filteredOrganizers.length, onTotalItemsChange]);
                ...
                >
                  ...
                  <td className="px-6 py-4 text-left">
                    <span className={`text-xs font-bold px-3 py-1 rounded-full border ${
                      organizer.role?.toLowerCase() === 'admin' 
                        ? 'bg-purple-100 text-purple-700 border-purple-200' 
                        : organizer.role?.toLowerCase() === 'organizer'
                        ? 'bg-blue-100 text-blue-700 border-blue-200'
                        : 'bg-emerald-100 text-emerald-700 border-emerald-200'
                    }`}>
                      {(organizer.role || 'organizer').toUpperCase()}
                    </span>
                  </td>
